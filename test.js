...
// Bài 3:
let a3=[1, 5, 4, 7, 9, 0, -10, 13, 93, 14, 15], tmp = [], minRange = null;
for(i = 0; i < a3.length; i++){
for(j=0; j < a3.length && j != i; j++){
if(minRange == null || Math.abs(a3[i] - a3[j]) < minRange) minRange = Math.abs(a3[i] -
a3[j]);
tmp.push({
...
let out = tmp.filter(e => e.range==1).map(e => e.val);
console.log(out);
